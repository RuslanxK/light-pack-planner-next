...
const InnerBag = ({bagData, items, session}) => {
  ...
  return (
    ...
    { items?.length ? <div className="side-bar-icon-mobile"><IconButton onClick={showHideSideBar} sx={{ width: "33.33%", height: "40px", zIndex: "99", borderRadius: "0px", position: "fixed", bottom: "0px", left: "0px", backgroundColor: theme.green, color: "white"}}>{showSideBarMobile === true ? <CloseIcon /> : <FlipCameraIosOutlinedIcon sx={{fontSize: "20px"}}/> }</IconButton></div> : null }
     <div className="share-icon-mobile"><Link href={`/share?id=${bagData.bag._id}`} target="_blank" rel="noopener noreferrer" underline="none"> <IconButton sx={{ width: "33.33%", height: "40px", zIndex: "99", borderRadius: "0px", position: "fixed", bottom: "0px", left: "33.3%", backgroundColor: theme.palette.primary.dark, color: "white"}}><ShareIcon sx={{fontSize: "20px"}}/></IconButton></Link></div> 
     <div className="switch-icon-mobile"><IconButton sx={{ width: "33.33%", height: "40px", zIndex: "99", borderRadius: "0px", position: "fixed", bottom: "0px", left: "66.6%", backgroundColor: theme.palette.secondary.dark, color: "white"}}> <Switch onChange={handleSwitchChange} sx={{transform: "scale(0.9)"}} checked={bagData.bag.exploreBags}/> </IconButton></div> 
